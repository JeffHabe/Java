...
public class AESAlgorithm {
    ...
    private byte[][] cipher(byte bytesMessage[][], int wordsKeyExpansion[]) {
        ...
        AESAlgorithm AESMessage=new AESAlgorithm();
        AESMessage.getTrace("Cipher Started Initial state", state);
        state = addRoundKey(state, wordsKeyExpansion, 0);
        AESMessage.getTrace("After addRoundKey", state);

        for (int round = 1; round <= Nr - 1; round++) {
            AESMessage.getTrace("Round " + round + " Started");
            state = subBytes(state);
            AESMessage.getTrace("After subBytes", state);
            state = shiftRows(state);
            AESMessage.getTrace("After shiftRows", state);
            state = mixColumns(state);
            AESMessage.getTrace("After mixColumns", state);
            state = addRoundKey(state, wordsKeyExpansion, round * Nb);
            AESMessage.getTrace("After addRoundKey", state);
//            System.out.println("===================");
        }
        AESMessage.getTrace("Final round Started");
        state = subBytes(state);
        AESMessage.getTrace("After subBytes", state);
        state = shiftRows(state);
        AESMessage.getTrace("After shiftRows", state);
...
//        System.out.println(Integer.toHexString(newWord));
        newWord <<= 8;
//        System.out.println(Integer.toHexString(newWord));
...
//        System.out.println(Integer.toHexString(newWord));
        newWord ^= (int) sboxTransform((byte) ((word & 0xff00) >>> 8)) &
                0x000000ff;
        System.out.println(Integer.toHexString((byte) ((word & 0xff00) >>> 8)& 0x000000ff));
        newWord <<= 8;
//        System.out.println(Integer.toHexString(newWord));
...
//    	System.out.println(Integer.toBinaryString((word <<8)));
//    	System.out.println(Integer.toHexString((word << 8)));
//    	System.out.println(Integer.toBinaryString((word >> 24)));
//    	System.out.println(Integer.toHexString((word >> 24)));
//    	int a=(word << 8) ^ ((word >> 24) & 0x000000ff);
...
//        System.out.println(Integer.toBinaryString(((int) b2) & 0x000000ff));
//        System.out.println(Integer.toHexString(((int) b2) & 0x000000ff));
//        System.out.println(Integer.toHexString(word));
...
//                System.out.println(Integer.toHexString(iTemp));
            } else if (Nk > 6 && i % Nk == 4) {
                iTemp = subWord(iTemp);
//                System.out.println(Integer.toHexString(iTemp));
            } // end if
    ...
    public byte[] createKey() {
        ...
        String keyStr[]=new String[]{"13","-61" ,"42" ,"-100"
        	,"80" ,"58", "110", "121"
        	,"-40" ,"57" ,"108" ,"-71"
        	,"20", "15", "-3", "80"
        	};
        for(int i=0;i<keyStr.length;i++)
        key[i]=Byte.parseByte(keyStr[i]);
        return key;
    }
    ...
    private byte[][] invCipher(byte bytesMessage[][], int wordsKeyExpansion[]) {
        ...
        AESAlgorithm AESMessage=new AESAlgorithm();
        AESMessage.getTrace("Inverse Cipher Started Initial state", state); // debug trace
        state = addRoundKey(state, wordsKeyExpansion, Nr * Nb);
        AESMessage.getTrace("After addRoundKey", state); // debug trace

        for (int round = (Nr - 1); round >= 1; round--) {
           AESMessage.getTrace("Round " + round + " started!"); // debug trace
            state = invShiftRows(state);
            AESMessage.getTrace("After invShiftRows", state); // debug trace
            state = invSubBytes(state);
            AESMessage.getTrace("After invSubBytes", state); // debug trace
            state = addRoundKey(state, wordsKeyExpansion, round * Nb);
            AESMessage.getTrace("After addRoundKey", state); // debug trace
            state = invMixColumns(state);
           AESMessage.getTrace("After invMixColumns", state); // debug trace
//           System.out.println("===================");
        }
        state = invShiftRows(state);
        AESMessage.getTrace("After invShiftRows", state); // debug trace
    ...
    public class AESBlocks {
        ...
        public byte[] getBytes() {
            ...
            int iCursor = 0;
            Iterator<byte[][]> iter = m_listBlocks.iterator();
            while (iter.hasNext()) {
    ...
    public void getTrace(String str, byte[][] state){
    	String ans=str + "\n";
    	 for(int i = 0; i < state.length; i++) { 
